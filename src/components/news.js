...
class News extends React.Component {
  ...
  componentDidMount = () => {
    this.setState({randomNewsTopic : local_data.news[Math.floor(Math.random() * local_data.news.length)]});
    const self = this
    ...
    {
      this.setInterval = setInterval(() => this.setState({randomNewsTopic : local_data.news[Math.floor(Math.random() * local_data.news.length)]}), 20 * 1000)
